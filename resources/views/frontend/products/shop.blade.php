...
@section('title', 'Shop | '.$seo->meta_title)
@section('content')
          ...
          <div id="collapseOne" class="collapse show" aria-labelledby="headingOne" data-parent="#accordion">
             <div class="card-body card-shop-filters">
              ...
              @foreach($allcategory as $key => $category)
                @php
                  $catpro=App\Product::where('is_deleted',0)->where('status',1)->Where('cate_id',$category->id)->count();
                @endphp
                <a href="{{route('website.category',$category->cate_slug)}}">
                  <div class="custom-control custom-checkbox" style="padding-left: 0 !important;">
                   <label class="" for="{{$category->id}}">{{$category->cate_name}} <span class="badge badge-success">{{$catpro}}</span> </label>
                  </div>
              </a>
       ...
       <div class="left-ad mt-4">
        <img class="img-fluid" src="http://via.placeholder.com/254x557" alt="">
           ...
           <div class="col-md-9">
              ...
              <div class="shop-head">
                 <a href="{{url('/')}}"><span class="mdi mdi-home"></span> Home</a> <span class="mdi mdi-chevron-right"></span> <a href="">Shop</a>
              ...
              <div class="row no-gutters" id="loadmoreproduct">

              </div>
              <nav>
                 <ul class="pagination justify-content-center mt-4">

                 <button class="btn btn-success" type="button" name="button" onclick="add_more()" id="loadbtn">Load More</button>
...
<script>
  ...
  function add_more(){
    if(i < 6){
      $.post('{{ route('shop.load.more.show') }}', {_token: '{{ csrf_token() }}',id:i},
          function(data) {
